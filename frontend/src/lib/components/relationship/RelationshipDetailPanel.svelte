<script lang="ts">
	...
	let { nodeId, embedded = false }: Props = $props();
...
{#snippet detailContent()}
	{#if node}
		<div class="mb-4 flex flex-col items-center text-center">
			{#if node.avatar}
				<img
					src={node.avatar}
					alt=""
					class="h-24 w-24 rounded-full border-2 border-[#8c5cfa]/40 bg-[#262933]"
				/>
			{/if}
				...
				<div class="space-y-3">
					{#each eliaRelationships as rel}
						<div>
							...
							<div class="h-1.5 rounded-full bg-[#262933]">
								<div
			...
			<section class="mb-5">
				...
				<div class="space-y-3">
					{#each relationshipHistory as item}
						<div class="border-l-2 border-[#8c5cfa]/40 pl-3">
				...
				<ul class="space-y-2 text-sm">
					{#each edges as edge}
						...
						<li class="flex items-center gap-2 text-text-secondary">
							<span
								class="h-2 w-2 rounded-full"
								style="background:{edgeStyles[edge.type].color}"
							></span>
							{other} — {edge.label}
						</li>
					{/each}
		...
		{/if}
	{:else}
		<div class="flex items-center justify-center p-6 text-center text-sm text-text-muted">
