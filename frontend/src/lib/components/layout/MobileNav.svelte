<script lang="ts">
	...
	const memberTabs = [
		...
		{ href: '/mypage', label: '마이', icon: User, match: '/mypage' }
	];

	const tabs = $derived(isLoggedIn() ? memberTabs : guestTabs);
</script>

<nav
	class="fixed inset-x-0 bottom-0 z-50 flex border-t border-white/10 bg-bg-surface/95 backdrop-blur-md lg:hidden"
>
	{#each tabs as tab}
		...
		<a
			href={tab.href}
			class="flex flex-1 flex-col items-center gap-0.5 py-2 text-[10px] transition {active
				? 'text-primary-400'
				: 'text-text-muted'}"
		>
	...
	{#if !isLoggedIn()}
		<a
			...
			class="flex flex-1 flex-col items-center gap-0.5 py-2 text-[10px] text-primary-300"
		>
			<User class="h-5 w-5" />
			로그인
		</a>
